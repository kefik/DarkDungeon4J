...
public class Test12_TrapAttakcs {
		...
		while (true) {
			...
			if (entityNum == 0) {
				...
				if (heroBody.atRoom.feature != null && heroBody.atRoom.feature.alive && heroBody.hand != null && heroBody.atRoom.item == null) {
					makeHeroDropSword(heroBody, heroViews.get(heroBody), roomsView.getRoomView(heroBody.atRoom.id));
				} else
				if (heroBody.atRoom.feature != null && heroBody.atRoom.feature.alive && heroBody.hand == null) {
					makeHeroAttackTrap(heroBody, heroViews.get(heroBody), roomsView.getRoomView(heroBody.atRoom.id));
				} else
				...
				if (entityNum < simState.monsters.size()) {
					...
					if (entityNum < simState.features.size()) {
						...
						if (trapBody.alive && trapBody.atRoom.monster != null) {
							makeTrapAttack(trapBody, roomsView.getRoomView(trapBody.atRoom.id), trapBody.atRoom.monster, monsterViews.get(trapBody.atRoom.monster));
