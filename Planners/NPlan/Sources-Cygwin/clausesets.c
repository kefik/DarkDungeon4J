...
int propagate(satinstance sati,int level) {
  ...
  while(startlong <= sati->endunitstack) {
    ...
    while(sati->startunitstack <= sati->endunitstack) {
      ...
      if(propagate2lit(sati,l,level)) return 1;
    }
...
/* TODO: Why does this go backwards? Fix. */

void undosetlits(satinstance sati) {
  int i;

  //  for(i=sati->endunitstack;i>=0;i--) litunset(sati,sati->unitstack[i]);
  for(i=0;i<=sati->endunitstack;i++) litunset(sati,sati->unitstack[i]);
...
int returnUCC(satinstance sati,int level) {
  ...
  for(i=0;i<sati->NofUCClauses;i++) {
    ...
    if(addtoqueue(sati,sati->UCC[i],REASON_INPUT,0) || propagate(sati,0)) return 1;
  }
...
int solve0(satinstance sati,int conflictstogo,int restart) {
  ...
  if(sati->notcalledbefore) {
    ...
    for(p=0;p<sati->initialunits;p++) {
      if(addtoqueue(sati,sati->initialunittable[p],REASON_INPUT,0)) {
	goto UNSAT;
      }
    }
  ...
  do {
    ...
    if(q || propagate(sati,sati->dlevel)) {	/* Got a conflict? */
      ...
      /* When exiting learn()l, CCreason points to the newly learned clause,
  ...
  if(restart) {
    ...
    setUPstacktop(sati,sati->declevels[0]-1);
